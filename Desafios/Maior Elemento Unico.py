...
try: 
    vezes = int(input("Quantos números você quer digitar na sua array? "))
    for i in range(vezes):
        adicionar = int(input("Digite os números da sua array: "))
        numeros.append(adicionar)
except ValueError:
    print("Evite digitar string aonde é int")
...
def maior_numero(a,b):
    for i in a:
        contagem = a.count(i)
        print(f"O número {i} aparece {contagem} vez na lista")
        if contagem == 1:
            b.append(i)
    if not b:
        return -1
...
print(f"O maior número unico do seu array é {maior_numero(numeros,unicos)}")
